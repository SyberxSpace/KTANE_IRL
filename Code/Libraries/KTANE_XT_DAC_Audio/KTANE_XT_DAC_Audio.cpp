// File: KTANE_XT_DAC_Audio.cpp
// Description: XTronical DAC Audio Library, currently supporting ESP32, with modifications
// Version: DAC Audio V4.2.1 - KTANE v1.0
// Last Update: May-18-2021
// This file, KTANE_XT_DAC_Audio.h, and MusicDefinitions.h were modified to add notes and an instrument for use in the KTANE_IRL project
// Modifications by Nicholas Schwankl (https://github.com/SyberxSpace)

// Original fine information below:
...
void XT_Instrument_Class::SetInstrument(uint16_t Instrument)
{
	...
	switch (Instrument)
	{
		...
		case(INSTRUMENT_SIMONSPEAK)		: SetSimonSpeakInstrument();break;
...
void XT_Instrument_Class::SetSimonSpeakInstrument()
{
	SetWaveForm(WAVE_SINE);
	FirstEnvelope=new XT_Envelope_Class();
	FirstEnvelope->AddPart(15,127);
	FirstEnvelope->AddPart(470,127);
	FirstEnvelope->AddPart(15,0);
}
